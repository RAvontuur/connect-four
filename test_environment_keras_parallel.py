...
print("[1] test O wins (vertical)")
print("[2] test illegal move X")
print("[3] test illegal move O")
env = ConnectFourEnvironmentKeras(parallel_plays=4)
env.ignore_invalid_actions = False
...
assert(env.get_player(play=2) == 1)
assert(np.any(env.terminated == 0))
assert(np.any(env.terminated == 1))
dummy_move = 3
env.move([dummy_move,6,dummy_move,0])
print(env.display(play=0))
print(env.display(play=1))
print(env.display(play=2))
print(env.display(play=3))

assert(np.all(env.terminated == 1))
assert(env.get_player(play=0) == 1)
assert(env.get_player(play=1) == -1)
assert(env.get_player(play=2) == 1)
assert(env.get_player(play=3) == -1)
assert(env.game_result(1, play=0) == 1)
assert(env.game_result(-1, play=0) == -1)
assert(env.game_result(1, play=1) == -1)
assert(env.game_result(-1, play=1) == 1)
assert(env.game_result(1, play=2) == -1)
assert(env.game_result(-1, play=2) == 1)
assert(env.game_result(1, play=3) == 1)
assert(env.game_result(-1, play=3) == -1)
